...
This guide outlines the changes and steps needed to migrate your codebase to the latest version of the Lorikeet TypeScript SDK.
...
To use it, upgrade the `@lorikeetai/node-sdk` package, then run `./node_modules/.bin/lorikeetai-node-sdk migrate ./your/src/folders` to update your code.
...
The `httpAgent` client option has been removed in favor of a [platform-specific `fetchOptions` property](https://github.com/optechai/node-sdk#fetch-options).
This change was made as `httpAgent` relied on `node:http` agents which are not supported by any runtime's builtin fetch implementation.

If you were using `httpAgent` for proxy support, check out the [new proxy documentation](https://github.com/optechai/node-sdk#configuring-proxies).
...
#### Refactor of `@lorikeetai/node-sdk/core`, `error`, `pagination`, `resource` and `uploads`

Much of the `@lorikeetai/node-sdk/core` file was intended to be internal-only but it was publicly accessible, as such it has been refactored and split up into internal and public files, with public-facing code moved to a new `core` folder and internal code moving to the private `internal` folder.
...
```typescript
// Before
import '@lorikeetai/node-sdk/error';
import '@lorikeetai/node-sdk/pagination';
import '@lorikeetai/node-sdk/resource';
import '@lorikeetai/node-sdk/uploads';

// After
import '@lorikeetai/node-sdk/core/error';
import '@lorikeetai/node-sdk/core/pagination';
import '@lorikeetai/node-sdk/core/resource';
import '@lorikeetai/node-sdk/core/uploads';
```

If you were relying on anything that was only exported from `@lorikeetai/node-sdk/core` and is also not accessible anywhere else, please open an issue and we'll consider adding it to the public API.

#### Resource classes

Previously under certain circumstances it was possible to import resource classes like `Conversation` directly from the root of the package. This was never valid at the type level and only worked in CommonJS files.
Now you must always either reference them as static class properties or import them directly from the files in which they are defined.

```typescript
// Before
const { Conversation } = require('@lorikeetai/node-sdk');

// After
const { Lorikeet } = require('@lorikeetai/node-sdk');
Lorikeet.Conversation; // or import directly from @lorikeetai/node-sdk/resources/conversation/conversation
```

#### Cleaned up `uploads` exports

As part of the `core` refactor, `@lorikeetai/node-sdk/uploads` was moved to `@lorikeetai/node-sdk/core/uploads`
...
```typescript
import { type Uploadable, toFile } from '@lorikeetai/node-sdk/core/uploads';
```
...
```typescript
// Before
import { APIClient } from '@lorikeetai/node-sdk/core';
...
```ts
// Before
Lorikeet.fileFromPath('path/to/file');
...
The `@lorikeetai/node-sdk/shims` imports have been removed. Your global types must now be [correctly configured](#minimum-types-requirements).

### `@lorikeetai/node-sdk/src` directory removed

Previously IDEs may have auto-completed imports from the `@lorikeetai/node-sdk/src` directory, however this
directory was only included for an improved go-to-definition experience and should not have been used at runtime.

If you have any `@lorikeetai/node-sdk/src/*` imports, you will need to replace them with `@lorikeetai/node-sdk/*`.

```ts
// Before
import Lorikeet from '@lorikeetai/node-sdk/src';
