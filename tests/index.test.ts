...
import { APIPromise } from '@lorikeetai/node-sdk/core/api-promise';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['LORIKEET_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Lorikeet) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['LORIKEET_LOG'] = 'debug';
      const client = new Lorikeet({
        logger: logger,
        clientID: 'My Client ID',
        clientSecret: 'My Client Secret',
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['LORIKEET_LOG'] = 'not a log level';
      const client = new Lorikeet({
        logger: logger,
        clientID: 'My Client ID',
        clientSecret: 'My Client Secret',
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'LORIKEET_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
